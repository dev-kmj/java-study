...
- 로컬 변수는 그 변수를 선언한 메서드 안에서만 살 수 있다.
      ...
      - s라는 변수는 read() 메서드 내에서만 쓸 수 있다. 즉, 변수의 영역은 그 메서드 내로 제한된다. 그 클래스에 있는 다른 어떤 코드에서도 s를 볼 수 없음
...
- 인스턴스 변수는 객체가 살아 있는 동안 계속 살 수 있다. 즉, 객체가 살아 있다면 그 인스턴스 변수도 살아 있음
      ...
      - s라는 변수의 영역은 setSize() 내부로 제한됨. 하지만 size라는 인스턴스 변수의 영역은 메서드가 아닌 객체 내부이기 때문에 객체와 생사를 함께함
